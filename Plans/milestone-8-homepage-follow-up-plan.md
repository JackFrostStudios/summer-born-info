...
- Replace the existing colour-mode `<select>` with a dedicated theme toggle control.
...
Given a keyboard or screen-reader user reaches the header controls, when they interact with the toggle, then it should expose a clear accessible name, state, focus styling, and predictable button semantics without relying on icon imagery alone.
...
4. Add the supplied `sun.svg` and `moon-stars.svg` assets to the production-served asset path and wire them into the theme control without adding a new icon dependency.
5. Implement motion for the icon transition that is lightweight, accessible, and compatible with reduced-motion expectations.
6. Update the homepage footer content so it includes the required icon attribution link while preserving the current project summary messaging.
7. Update or add tests that cover shell structure, theme toggle behaviour, and footer attribution rendering.
8. Refresh i18n extraction outputs for any new visible text such as the attribution text.
9. Run the required UI validation commands before handoff.
...
- `ThemeToggle`
  Owns the interactive UI for explicit light/dark switching, icon rendering, and animation state.
- `ColourModeService`
...
5. Implement toggle interaction and icon animation on top of the existing colour-mode service.
...
- Risk: A two-state toggle makes the initial system-following behaviour less obvious.
  - Mitigation: keep system preference as the default only when no stored override exists, and cover that behaviour in service and component tests.
...
- [x] Accessibility semantics and initial system-following behaviour are covered in unit tests where practical, with browser-level focus and reduced-motion verification left as manual follow-up.
